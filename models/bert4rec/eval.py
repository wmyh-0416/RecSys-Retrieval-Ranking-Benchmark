...
def append_mask_for_next_item(seq: torch.Tensor, mask_id: int) -> torch.Tensor:
    # Build eval input: keep history items, append [MASK] at the end.
    # If sequence is already full, drop the oldest item.
    batch_size, max_len = seq.shape
    out = torch.zeros_like(seq)
    mask_token = seq.new_full((1,), int(mask_id))

    for b in range(batch_size):
        tokens = seq[b][seq[b] > 0]
        if tokens.numel() >= max_len:
            tokens = tokens[-(max_len - 1) :]
        new_tokens = torch.cat([tokens, mask_token], dim=0)
        out[b, -new_tokens.numel() :] = new_tokens
    return out
...
def evaluate_model(
    ...
    for seq, candidates in data_loader:
        ...
        eval_seq = append_mask_for_next_item(seq, mask_id=mask_id)
        hidden_states = model(eval_seq)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate BERT4Rec with unified benchmark protocol.")
...
def main():
    ...
    valid_metrics = evaluate_model(
        ...
        device=device,
        mask_id=int(model_cfg.get("mask_id", data.mask_id)),
        topk=(5, 10, 20),
    )
