...
class SASRec(nn.Module):
    def __init__(
        ...
        # Disable nested tensor path for better device compatibility (notably MPS).
    ...
    def forward(self, seq: torch.Tensor) -> torch.Tensor:
        # seq: [B, L], item ids with 0 as padding id.
        ...
        # Keep padded tokens as zero vectors before entering Transformer.
    ...
    def score_candidates(self, hidden: torch.Tensor, candidates: torch.Tensor) -> torch.Tensor:
        ...
        scores = torch.bmm(candidate_emb, hidden.unsqueeze(-1)).squeeze(-1)
        return scores
